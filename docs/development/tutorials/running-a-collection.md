...
Once you understand how to run it for a Collection then it can be applied to any to debug errors that may have happened over night. It would be good§ to read the key  concepts in the  data operations manual for clarity on the  terms that we use. this tutorial will describe the practical applications of these concepts.

### Anatomy of a collection repository

First it's good to understand the anatomy of a collection. these fies may not exist as they are generated when initialising and running the collection.

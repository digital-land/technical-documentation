...
When the collection is added to the specification, this should automatically create a new DAG in the Airflow environments permitted by the `environment` field you set in Step 3.
...
If you don't want the collection to run in production yet, set `environment` to `staging` or `development` in the dataset specification (Step 3) this will ensure that the new dataset will only apprear in lower environments.
